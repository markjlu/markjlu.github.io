...
G.util.token = {
	...
	addToken : function(url,type){
		//type标识请求的方�?,jq标识jquery，lk标识普通链�?,fr标识form表单,ow打开新窗�?
		var token=this.getToken();
		//只支持http和https协议，当url中无协议头的时候，应该检查当前页面的协议�?
...
G.util.localShare = (function(){
	...
	return function(fn) {
		...
		if (_scriptLoaded == 2 && _localCache) { // 加载已完�?
			_queue.exec(_localCache);
			return;
		}

		if (_scriptLoaded == 1) { // 加载�?
			return;
		}
...
(function(G, $, undefined){
	function tip(opt){
        ...
        opt = jQuery.extend({}, {
			"position" : "rightBottom",  // 提示tip相对于target的位�?, 可选：'leftTop','rightTop','rightBottom','leftBottom'
			"distance" : 20,  //尖角相对于tip顶点的距�?
			"width" : "120",
			"html" : "",	//提示信息
			"target" : null,	//tip相对停靠的节�?, 类型�? selector
			"buttons" : null,	//tip中的按钮文字, �? ['确定', '取消']
			"group" : null,		//tip所属的组，当设定了这个参数，属于同一组的tip将在document中最多显示一�?
			"className" : "global_tip",	//tip最外层 节点的样�?
			"time" : null // time毫秒后自动关�?, 当存在按钮时，该参数无效
		//  "click_1" : function(){}	//点击第n个按钮时的回调函�?, �?1开�?
		}, opt || {});
